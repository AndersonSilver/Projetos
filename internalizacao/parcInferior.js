async function parcelamentoInferior() {
  ...
  var parcelaAtual = ticket.custom.contestacao_transacao_credito.parcelaAtual;
  const dataTrasacao = ticket.custom.contestacao_transacao_credito.data;

  try {
    const parcelas = [];

    for (let parcela = parseInt(parcelaAtual) - 1; parcela >= 1; parcela--)
      parcelas.push(`${parcela}`);
    const apiCallsResponses = [];
    let x = parseInt(parcelaAtual);
    for (let parcelaAtual of parcelas) {
      ...
      apiCallsResponses.push(result.data.data);
    }
    return apiCallsResponses;
  } catch (error) {
