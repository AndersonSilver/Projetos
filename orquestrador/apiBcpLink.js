...
async function getdocumentsBcp() {
  ...
  const id_campanha = ticket.payloadOrigin.data.id_campanha;
  const cartaoexpiracao = ticket.payloadOrigin.data.cartaoExpiracao;
  const emailCorretor = ""; // colocar a variavel que vai estar o email do
  ...
  const getToken = async () => {
    ...
    try {
      ...
      throw new Error("error get token", err);
    }
  };
  ...
  const getPortoSeguroBcpDadosPessoais = async () => {
    try {
      ...
      if (!response) {
        console.log("Nenhum dado de pessoa encontrado");
    ...
    if (!bcpData.email && !emailTicket) {
      ...
      const filtraDadosTickets = async () => {
        try {
          ...
          throw errorFactory("error validating ticket data", err);
        }
      };

      const filtroDados = await filtraDadosTickets();

      const getTypeEmails = async () => {
        if (id_campanha === "1") {
          const template_email = "rec_01";
          return {
            ...filtroDados,
            template_email,
            emailCorretor,
          ...
          const template_email = "venc_01";
          return {
            ...filtroDados,
            template_email,
            emailCorretor,
          };
        } else if (id_campanha == "3" && cartaoexpiracao === "15") {
          const template_email = "venc_01";
          return {
            ...filtroDados,
            template_email,
        ...
        } else if (id_campanha == "3" && cartaoexpiracao === "0") {
          const template_email = "venc_02";
          return {
            ...filtroDados,
            template_email,
            emailCorretor,
          };
        } else if (id_campanha == "4") {
          const template_email = "pag_01";
          return {
            ...filtroDados,
            template_email,
            emailCorretor,
          };
        } else if (id_campanha === "2") {
          const template_email = "cr_01";
          return {
            ...filtroDados,
            template_email,
            emailCorretor,
      ...
      const response = await getTypeEmails();
