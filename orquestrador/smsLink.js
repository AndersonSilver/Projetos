...
async function smsLink() {
  ...
  const link = ticket.payloadOrigin.data.link;
  ...
  const shortenLink = async () => {
    try {
      const { data } = await axios({
        ...
        headers: {
          Authorization: `Bearer ${configObject.bitlyToken}`,
        },
      });
      return data.link;
    } catch (err) {
      throw new Error("eror shorten link", err);
    }
  };
  ...
  const shortLink = await shortenLink(
    "https://google.com",
    configObject.bitlyToken
  );

  const menssagem = {
    destination: telefone,
    messageText: `Porto: Não foi possível fazer a cobrança de R$${ticket.payloadOrigin.data.valorContrato}, no cartão com numeração final ${ticket.payloadOrigin.data.finalCartao}. Você pode trocar o cartão, em ${shortLink}`,
  };
  ...
  const sendMessageViaSMS = async () => {
    try {
      ...
      throw new Error("error send sms", err);
    }
  };
