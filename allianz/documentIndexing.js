...
async function documentIndexing() {
    const env = "hml";
    ...
    async function sendDocument(env) {
      const getAmbientNotasGO = async () => {
        const url = {
          prod: "",
          hml: "https://eu-uat-brazil.apis.allianz.com/v1/push-to-pull/documentos/tipo-doc/",
        }[env];
        return url;
      };
    ...
    const documentIndexing = await sendDocument(env);
    console.log(documentIndexing);
