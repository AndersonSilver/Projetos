...
async function apiPostFlash() {
  ...
  const filtroEmbossadora = async () => {
    if (ticket.payloadOrigin.embossing.embossadora == "0015") {
      const ar = ticket.payloadOrigin.embossing.ar;
      return ar.substring(2);
    } else {
      return ticket.payloadOrigin.custom.embossing.ar;
    }
  };

  const codigoPesquisa = await filtroEmbossadora();
